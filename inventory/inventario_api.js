 ...
 *
 * IMPORTANTE: coloque inventario.html na mesma pasta deste arquivo.
 * O Node vai servir o HTML automaticamente — NÃO abra o .html direto
 * pelo explorador de arquivos, acesse sempre pelo navegador via
 * http://localhost:3000
 */
...
// MariaDB retorna BigInt em COUNT/SUM — converte para Number na serialização
...
// Coloque o inventario.html na mesma pasta deste arquivo.
...
app.get("/dashboard", async (_, res) => {
  try {
    const rowsTotais  = await query("SELECT COUNT(*) AS total, SUM(quantidade_disp) AS disp, SUM(quantidade_total - quantidade_disp) AS em_uso FROM itens WHERE condicao != 'Descartado'");
    const rowsAbertos = await query("SELECT COUNT(*) AS total FROM vw_emprestimos_abertos");
    const rowsDefeito = await query("SELECT COUNT(*) AS total FROM itens WHERE condicao = 'Defeituoso'");
    ...
    const abertos = rowsAbertos[0] || {};
    const defeito = rowsDefeito[0] || {};

    res.json({
      totais: {
        total:  Number(totais.total  ?? 0),
        disp:   Number(totais.disp   ?? 0),
        em_uso: Number(totais.em_uso ?? 0),
      },
      emprestimosAbertos: Number(abertos.total ?? 0),
      defeituosos:        Number(defeito.total  ?? 0),
      porCategoria: porCateg.map(c => ({ nome: c.nome, qtd: Number(c.qtd) })),
    });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
